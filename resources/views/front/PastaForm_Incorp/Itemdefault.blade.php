...
<div class="corpo-das-frases page-break {{$recuo}}">
    ...
    @foreach ($registro as $item)
        ...
        $contaLinhas++;
        $classtoggleImage = "";
        $frase_id = $item["frase_id"] ? $item["frase_id"] : 0;
        ...
        <div class="frase-box">      
            ...
            <div  data-index="{{$contaLinhas-1}}" data-indexid="{{$item["id"]}}" data-tipo="without-icon"  data-id="frase_{{$item["id"]}}" data-fraseId="{{$item["frase_id"] ?? 0}}" class="magic-input-selector without-icon frase frase_{{$item["id"]}} @if ($item["frase_id"]) noEdit @endif" id="txt_frase_{{$item["id"]}}">{{$item["frase"]}}</div>
            @if (!$frase_id)
                @include('front.includes.formulario.Textareamagic',  [
                'input' => 'frase_'. $item["id"], 
                'dataIndex' => $contaLinhas-1,
                'class' => 'magic-input-frase without-icon',
                'value' => $item["frase"],
                'placeholder' => 'Digite aqui a sua ideia, a sua inspiração!',
                ])                
            @endif
            
            <div data-id="autor_{{$item["id"]}}"  data-indexid="{{$item["id"]}}" data-tipo="without-icon"  class="magic-input-selector without-icon autor_{{$item["id"]}} autor  @if ($item["frase_id"]) noEdit @endif">
                {{$autor ?? "sem autor"}}
            </div>                
            @if (!$frase_id)
                @include('front.includes.formulario.Inputmagic',  [
                    ...
                    'value' => $item["autor"],
                    'dataIndex' => $contaLinhas-1,
                    'class' => "autor_input magic-input-autor web_pastas_back_form",
            ...
            <div class="no-print">
                    ...
                    <ul id="ul_edicao_{{$item["id"]}}" class="frases_em_edicao "> {{--frase editável--}}                        
                        <li>
                            <a  href="#" title="Cancelar"
                                ...
                                <span class="ico ico-cancelar-edicao-frases"></span>
                            </a>
                        </li>
                        <li>
                            <a href="" 
                                title="Salvar!"                                 
                                data-id="{{$item["id"]}}"                                
                                class="bt-salvar-frase-editada edit_id{{$item["id"]}} ativo"><span class="ico ico-salvar-edicao-frases {{$classtoggleImage}}"></span></a>
                        </li>
                    </ul>
                    <ul id="ul_view_{{$item["id"]}}" class="pastas editavel ativo"> {{--frase editável--}}
                        <li>
                            <a href="" 
                                ...
                                data-id="{{$item["id"]}}"
                                data-index="{{$contaLinhas-1}}"
                                class="magic-input-selector bt-edit-frase-fox item-tools toggle-edit-frase {{$classtoggleImage}} edit_id{{$item["id"]}}"><span class="ico ico-pencil {{$classtoggleImage}}"></span></a>
                            <a href="#" 
                ...
                <div class="ordenacao @if ($item["capa"]) comcapa @endif  @if ($contaLinhas>1 && $contaLinhas < count($registro)) ambos @endif    ">
                    <ul>
                        @if ($contaLinhas>1)
                            <li><a href="#" data-index="{{$contaLinhas-1}}" data-ordem="{{$$item["ordem"]}}" class="bt_sobe" title="Sobe"><span class="ico ico-seta-up"></span></a></li>
                        @endif
                        @if ($contaLinhas < count($registro))
                            <li><a href="#" data-index="{{$contaLinhas-1}}" data-ordem="{{$$item["ordem"]}}" class="bt_desce" title="Desce"><span class="ico ico-seta-down"></span></a></li>                            
                        @endif
                    </ul>                    
                </div>
            </div>
