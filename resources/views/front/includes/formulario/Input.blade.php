...
<div class="{{ $inputCol ?? "col-lg-10" }} field-input" id="field-input_{{$input ?? null}}">
    ...
    @if (isset($vmodel))        
        ...
        <input type="text" 
            ...
            id="{{ $input ?? null }}" 
            class="fadeIn second {{ $class ?? null }} @php if (isset($msgValidacao)) echo "erro"; @endphp"
            name="{{ $input ?? null}}" 
    ...
    @else    
        <input type="text" 
            ...
            ref="ref_{{ $input ?? null }}"
            id="{{ $input ?? null }}" 
            class="fadeIn second {{ $class ?? null }} @php if (isset($msgValidacao)) echo "erro"; @endphp @error($input) is-invalid @enderror" 
            name="{{ $input ?? null}}"
            placeholder="{{ $placeholder ?? null}}"
            value="{{$oldValue ?? $value}}"
            @if (isset($ref))                        
