...
<div class="tools no-print">
    <ul>
            ...
            @if (isset($logado))
                ...
                @else
                    <a href="/login" rel="nofollow"
                        @if (!$amp) idData="{{ $item->frase_id }}" 
                        data="go" @endif
                        title="Adicionar a suas Frases Favoritas."
                        @if (!$amp) class="padrao-icone favoritas fraseFavorita favorita-id-{{ $item->frase_id }}" @endif
                        >
                ...
                @endif
            @else
                <a href="/login" @if (!$amp) data="go" @endif rel="nofollow"
                    title="Que tal salvar esta frase em suas Frases Favoritas?"
                    @if (!$amp)  class="padrao-icone callLogin_ item-tools favoritas icon favorita-id-{{ $item->frase_id }} mg0" @endif
                    >
                    @if ($amp)
                        {{-- <amp-img alt="seguir" src="/images/icones/24x24/seguir-v01.svg" width="24px" height="24px"></amp-img> --}}
                    @else
        ...
        @if ($item->status == '1')
            <li>
                <a href="/frase/{{ $item->frase_id }}"
                    ...
                    >
                    @if ($amp)
                        {{-- <amp-img alt="seguir" src="/images/icones/24x24/imagem-v01.svg" width="24px" height="24px"></amp-img> --}}
                    @else
                        <span data-src="/images/icones/24x24/imagem-v01.svg" class="ico lazy-background lazy-imagem"></span>
                    @endif
        ...
        <li>
            <a href="#" title="Copiar"
                @if (!$amp) idData="{{ $item->item_id }}" @endif
                @if (!$amp)
